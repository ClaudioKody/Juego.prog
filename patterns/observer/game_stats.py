import os
from patterns.observer.game_stats import Subject
...
class GameStats(Subject):
    ...
    def next_level(self):
        """Pasa de nivel infinitamente incrementando la dificultad y la
        meta."""
        self._level += 1
        self._enemies_killed = 0
        self._enemies_target = 10 + (self._level - 1) * 5
        self.notify(score=self._score, lives=self._lives,
        level=self._level)
